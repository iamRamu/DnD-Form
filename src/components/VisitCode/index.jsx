...
import './index.css'
const VisitCode = props => {
  const {tests} = props
  console.log("visitCode Tests", tests, tests.length)
  const isTestsAvailable = tests.length > 0

  const handleEachTest = e => {
    e.preventDefault()
  }

  return(
    <div>
      <h3 className="visitcode-title">Visit Code</h3>
      {isTestsAvailable &&
        <div className="check-box-area" onClick={handleEachTest}>
          {tests.map(eachTest => 
            <div key={eachTest.id} className="check-box-area-in">
              <input type="checkbox"  id={eachTest.id} />
              <label htmlFor={eachTest.id}>{eachTest.name}</label>
            </div>
          )}
